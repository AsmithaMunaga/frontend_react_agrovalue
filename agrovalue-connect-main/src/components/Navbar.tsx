      ...
      <div className="container mx-auto px-4">
        <div className="flex items-center justify-between h-16">
          ...
          <Link to="/" className="flex items-center gap-2 group">
            ...
            <div>
              <span className="font-display text-xl font-bold text-primary-foreground leading-none block">AgroValue</span>
              <span className="text-[10px] text-primary-foreground/60 font-body">Rural Marketplace</span>
            </div>
          </Link>
          ...
          <div className="flex items-center gap-2">
            {/* Cart */}
            <Link to="/cart" className="relative p-2 text-primary-foreground/80 hover:text-primary-foreground hover:bg-primary/20 rounded-lg transition-all">
            ...
            {isAuthenticated && user ? (
                ...
                <DropdownMenuTrigger asChild>
                  <button className="flex items-center gap-2 px-3 py-1.5 bg-primary/20 hover:bg-primary/30 rounded-lg transition-all">
                    <div className="w-6 h-6 rounded-full bg-gradient-harvest flex items-center justify-center">
                      <span className="text-xs font-bold text-soil">{user.name[0]}</span>
                    </div>
                    <span className="text-sm text-primary-foreground hidden sm:block">{user.name.split(' ')[0]}</span>
                  </button>
                </DropdownMenuTrigger>
              ...
              <div className="flex gap-2">
                <Button variant="ghost" size="sm" className="text-primary-foreground hover:bg-primary/20" onClick={() => navigate('/login')}>
                  Login
                </Button>
                <Button size="sm" className="bg-gradient-harvest text-soil font-semibold hover:opacity-90 shadow-amber border-0" onClick={() => navigate('/register')}>
                  Join Now
                </Button>
              </div>
            )}
            ...
            {/* Mobile toggle */}
            <button className="md:hidden p-2 text-primary-foreground/80 hover:bg-primary/20 rounded-lg" onClick={() => setMobileOpen(!mobileOpen)}>
